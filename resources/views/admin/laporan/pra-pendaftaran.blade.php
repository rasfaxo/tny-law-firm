    ...
    <div class="space-y-5">
        ...
        <div class="bg-white border border-[#e2e8f0] rounded-[16px] p-5 shadow-sm space-y-3 no-print">
            ...
            <div class="flex flex-wrap gap-2.5">
                <a href="{{ route('admin.laporan.pra-pendaftaran') }}" class="inline-flex items-center gap-2 bg-[#1e3a8a] text-white border border-[#1e3a8a] text-[13px] font-semibold px-4 py-2 rounded-[12px] transition shadow-xs">
        ...
        <div class="bg-white border border-[#e2e8f0] rounded-[16px] p-6 shadow-sm no-print">
            <form method="GET" action="{{ route('admin.laporan.pra-pendaftaran') }}" class="space-y-5">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div>
                        <label for="status_pengajuan" class="block text-[10px] font-semibold text-[#64748b] tracking-[0.25px] uppercase mb-2">Status Pengajuan</label>
                ...
                <div class="flex flex-wrap items-center gap-3 pt-4 border-t border-[#e2e8f0]">
                    ...
                    <a href="{{ route('admin.laporan.pra-pendaftaran') }}" class="bg-white border border-[#e2e8f0] hover:bg-gray-50 text-[#334155] rounded-[12px] h-11 px-5 text-[13px] font-semibold transition shadow-sm inline-flex items-center gap-2">
                    ...
                    <a href="{{ route('admin.laporan.pra-pendaftaran.cetak', request()->query()) }}" target="_blank" class="bg-emerald-600 hover:bg-emerald-700 text-white rounded-[12px] h-11 px-5 text-[13px] font-semibold transition shadow-sm inline-flex items-center gap-2 ml-auto">
                        <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white border border-[#e2e8f0] rounded-[16px] shadow-sm overflow-hidden print-area">
            <div class="p-6 border-b border-[#f1f5f9] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h3 class="font-bold text-[#0f172a] text-[16px]">{{ __('Laporan Pra-Pendaftaran Perkara') }}</h3>
                ...
                <table class="min-w-full divide-y divide-[#e2e8f0]">
                    <thead class="bg-[#f8fafc]">
                        <tr>
                            ...
                            <th class="px-5 py-3.5 text-right text-[10px] font-semibold text-[#64748b] uppercase tracking-[0.25px] no-print">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-[#e2e8f0]/60">
                        @forelse ($laporan as $pengajuan)
                            <tr class="hover:bg-[#f8fafc] transition duration-150">
                                ...
                                <td class="px-5 py-4 whitespace-nowrap text-right text-[13px] font-semibold no-print">
