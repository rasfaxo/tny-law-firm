...
This file is only an adapter for GPT-5.5. Do not treat this file as a replacement for `AGENTS.md`.

GPT must read `AGENTS.md` first before following this adapter file.
...
## GPT-Specific Working Rules

GPT must prioritize reasoning, consistency, and safety.

Before producing code, GPT should:

1. Check whether the requested change conflicts with `AGENTS.md`.
2. Identify the exact scope of the task.
3. Identify the exact files affected.
4. Explain assumptions clearly.
5. Avoid inventing database columns, routes, models, or features.
6. Prefer the smallest safe implementation.
7. Keep the implementation aligned with the locked thesis design.

When reviewing code, GPT should check:
...
* Transaction safety
* Manual testing completeness

When debugging, GPT should:
...
* Explain conflicts between Laravel defaults and the thesis design before suggesting changes.
...
8. Ask for confirmation before making any changes.
9. Never make changes without consulting the locked thesis design.

---

## Final Reminder

GPT must not override the locked thesis design.
