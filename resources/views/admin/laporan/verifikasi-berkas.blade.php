    ...
    <div class="space-y-5">
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-xl p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
            <div class="space-y-1.5">
                ...
                <p class="text-sm text-gray-500 leading-normal">
                    Rekapitulasi riwayat pemeriksaan dan verifikasi dokumen perkara oleh Staf Legal.
                </p>
            </div>
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-xl p-5 shadow-sm space-y-3 no-print">
            ...
            <div class="flex flex-wrap gap-2.5">
                ...
                <a href="{{ route('admin.laporan.verifikasi-berkas') }}" class="inline-flex items-center gap-2 bg-navy-primary text-white border border-navy-primary text-sm font-semibold px-4 py-2 rounded-xl transition shadow-xs">
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-xl p-6 shadow-sm no-print">
            <form method="GET" action="{{ route('admin.laporan.verifikasi-berkas') }}" class="space-y-5">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div>
                        ...
                        <x-select id="status_verifikasi" name="status_verifikasi" class="w-full text-navy-dark">
                    ...
                    <div>
                        <x-input-label for="id_staf_legal" :value="__('Staf Legal')" />
                        <x-select id="id_staf_legal" name="id_staf_legal" class="w-full text-navy-dark">
                ...
                <div class="flex flex-wrap items-center gap-3 pt-4 border-t border-[#E2E8F0]">
                    ...
                    <x-secondary-button href="{{ route('admin.laporan.verifikasi-berkas') }}" tag="a" class="gap-2">
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-xl shadow-sm overflow-hidden print-area">
            <div class="p-6 border-b border-[#F1F5F9] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h3 class="font-bold text-navy-dark text-lg">{{ __('Laporan Verifikasi Berkas') }}</h3>
                ...
                <table class="min-w-full divide-y divide-[#E2E8F0]">
                    <thead class="bg-[#F8FAFC]">
                        <tr>
                            ...
                            <th class="px-5 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Kode</th>
                            <th class="px-5 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Nama Klien</th>
                            <th class="px-5 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Judul Perkara</th>
                            <th class="px-5 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Kategori</th>
                            <th class="px-5 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Staf Legal</th>
                            <th class="px-5 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Status Verifikasi</th>
                            <th class="px-5 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Tanggal Verifikasi</th>
                            <th class="px-5 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Catatan Umum</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-[#E2E8F0]">
                        @forelse ($laporan as $verifikasi)
                            ...
                            <tr class="hover:bg-[#F8FAFC] transition duration-150">
                                ...
                                <td class="px-5 py-4 whitespace-nowrap text-sm font-bold text-accent-blue font-mono">
                                    VR-{{ str_pad($verifikasi->id_verifikasi, 3, '0', STR_PAD_LEFT) }}
                                </td>
                                <td class="px-5 py-4 whitespace-nowrap text-sm font-semibold text-navy-dark">
                                    {{ $pengajuan?->klien?->nama ?? '-' }}
                                </td>
                                <td class="px-5 py-4 text-sm text-gray-700 truncate max-w-xs font-medium">
                                    {{ $pengajuan?->judul_perkara ?? '-' }}
                                </td>
                                <td class="px-5 py-4 whitespace-nowrap text-sm text-gray-700">
                                    {{ $pengajuan?->kategori?->nama_kategori ?? '-' }}
                                </td>
                                <td class="px-5 py-4 whitespace-nowrap text-sm font-medium text-navy-dark">
                                    {{ $verifikasi->stafLegal?->nama ?? '-' }}
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="px-5 py-12 text-center">
                                    <x-empty-state title="Tidak Ada Data" message="Tidak ada data verifikasi berkas sesuai filter yang dipilih." />
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
