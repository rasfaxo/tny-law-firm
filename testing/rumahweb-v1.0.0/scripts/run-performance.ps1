...
if ($targetHost -ne 'retest.tnypartners.com') { throw "Harness is locked to retest.tnypartners.com, not '$targetHost'." }
$authorization = Require-Environment 'RETEST_PROVIDER_AUTHORIZATION'
$testerLocation = Require-Environment 'RETEST_TESTER_LOCATION'
$requiredSecrets = @('RETEST_BASIC_USER','RETEST_BASIC_PASSWORD','JMETER_CLIENT_EMAIL','JMETER_CLIENT_PASSWORD','JMETER_LEGAL_EMAIL','JMETER_LEGAL_PASSWORD','RETEST_CATEGORY_ID','RETEST_VALID_DOCUMENT')
...
$runDirectory = if ([string]::IsNullOrWhiteSpace($RunDirectory)) {
    Join-Path $OutputRoot "$RunId-rumahweb-v100"
} else {
...
$manifest = [ordered]@{
    run_id = $RunId
    environment = 'private-rumahweb-clone'
