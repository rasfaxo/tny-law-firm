...
foreach ($file in $expected.Keys) {
    ...
    if ($raw -notmatch 'RETEST_TARGET_HOST' -or $raw -notmatch 'RETEST_BASIC_USER') { throw "$file lacks isolated-host or Directory Privacy parameters." }
    if (@($xml.SelectNodes("//JSR223PreProcessor[@testname='Safety Gate - Private Retest Only']")).Count -ne 1) { throw "$file lacks the in-plan production safety gate." }
...
$cloneEnv = Get-Content -LiteralPath (Join-Path $suiteRoot 'templates\clone-env.template') -Raw
foreach ($requiredValue in @(
    'APP_URL=https://retest.tnypartners.com',
    'DB_DATABASE=tnym6311_tnyretest',
    'MAIL_FROM_ADDRESS=no-reply@retest.tnypartners.com',
    ...
    'TRUSTED_HOSTS=retest.tnypartners.com'
)) {
    if (-not $cloneEnv.Contains($requiredValue)) { throw "Clone environment template is missing: $requiredValue" }
}
if ($cloneEnv -match '(?m)^(APP_KEY|DB_PASSWORD|ADMIN_DEFAULT_PASSWORD|RESEND_API_KEY|AZURE_STORAGE_CONNECTION_STRING)=\S+') {
    throw 'Clone environment template contains a secret-like value.'
}
Write-Host 'OK isolated clone environment template'
