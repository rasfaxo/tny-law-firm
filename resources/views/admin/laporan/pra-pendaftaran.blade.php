    ...
    <div class="space-y-5">
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-xl p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
            <div class="space-y-1.5">
                ...
                <p class="text-sm text-gray-500 leading-normal">
                    Rekapitulasi berkas pra-pendaftaran perkara yang diajukan oleh Klien, lengkap dengan status verifikasi dan kategori.
                </p>
            </div>
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-xl p-5 shadow-sm space-y-3 no-print">
            ...
            <div class="flex flex-wrap gap-2.5">
                <a href="{{ route('admin.laporan.pra-pendaftaran') }}" class="inline-flex items-center gap-2 bg-navy-primary text-white border border-navy-primary text-sm font-semibold px-4 py-2 rounded-xl transition shadow-xs">
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-xl p-6 shadow-sm no-print">
            <form method="GET" action="{{ route('admin.laporan.pra-pendaftaran') }}" class="space-y-5">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div>
                        <x-input-label for="status_pengajuan" :value="__('Status Pengajuan')" />
                        <x-select id="status_pengajuan" name="status_pengajuan" class="w-full text-navy-dark">
                    ...
                    <div>
                        ...
                        <x-select id="id_kategori" name="id_kategori" class="w-full text-navy-dark">
                ...
                <div class="flex flex-wrap items-center gap-3 pt-4 border-t border-[#E2E8F0]">
                    ...
                    <a href="{{ route('admin.laporan.pra-pendaftaran.cetak', request()->query()) }}" target="_blank" rel="noopener noreferrer" class="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl h-11 px-5 text-sm font-semibold transition shadow-sm inline-flex items-center gap-2 ml-auto">
                        <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <table class="min-w-full divide-y divide-[#E2E8F0]">
                    <thead class="bg-[#F8FAFC]">
                        <tr>
                            ...
                            <th class="px-5 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Tanggal Pengajuan</th>
                            <th class="px-5 py-3.5 text-right text-xs font-bold text-gray-400 uppercase tracking-wider no-print">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-[#E2E8F0]">
                        @forelse ($laporan as $pengajuan)
                            <tr class="hover:bg-[#F8FAFC] transition duration-150">
                                ...
                                <td class="px-5 py-4 whitespace-nowrap text-right text-sm font-semibold no-print">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-5 py-12 text-center">
                                    <x-empty-state title="Tidak Ada Data" message="Tidak ada data pra-pendaftaran sesuai filter yang dipilih." />
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
