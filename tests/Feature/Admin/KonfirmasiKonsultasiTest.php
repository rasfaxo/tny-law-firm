...
class KonfirmasiKonsultasiTest extends TestCase
{
    ...
    public function test_identical_reconfirmation_does_not_duplicate_email_or_audit_event(): void
    {
        Notification::fake();
        $admin = $this->createAdmin();
        $booking = $this->createBookingAktif(null, null, null, [
        ...
        $payload = ['link_konsultasi' => 'https://meet.example.test/abc'];

        $this->actingAs($admin)
            ->patch(route('admin.booking-konsultasi.konfirmasi', $booking), $payload)
            ->assertRedirect();
        $this->actingAs($admin)
            ->patch(route('admin.booking-konsultasi.konfirmasi', $booking), $payload)
            ->assertRedirect();

        Notification::assertSentToTimes($booking->klien, ConsultationStatusNotification::class, 1);
