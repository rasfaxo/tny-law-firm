            ...
            <div class="bg-white border border-[#E2E8F0] p-6 sm:p-8 rounded-2xl shadow-sm">
                <form method="POST" action="{{ route('admin.kategori-perkara.store') }}" class="space-y-6">
                    ...
                    <div class="flex items-center justify-end gap-3 pt-2">
                        <a href="{{ route('admin.kategori-perkara.index') }}" class="text-sm text-gray-600 hover:text-gray-900 font-medium transition">
                            {{ __('Batal') }}
                        </a>
                        <x-primary-button class="bg-[#1e3a8a] hover:bg-blue-900 transition">{{ __('Simpan') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
