...
<head>
    ...
    <title>{{ $title ?? 'Cetak Laporan' }} - TNY Law Firm</title>
    ...
    <header class="no-print sticky top-0 z-50 bg-[#0F1E3A] text-white border-b border-slate-700 shadow-md">
        <div class="max-w-5xl mx-auto px-6 py-3 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <span class="font-extrabold text-base tracking-wider text-white">TNY LAW FIRM</span>
                <span class="text-xs text-slate-300 hidden sm:inline">• Pratinjau Dokumen Cetak</span>
            </div>
            <div class="flex items-center gap-3">
                <button type="button" onclick="window.close()" class="px-4 py-2 text-xs font-semibold rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 transition">
                    ← Tutup Tab
                </button>
                <button type="button" onclick="window.print()" class="px-5 py-2 text-xs font-bold rounded-lg bg-blue-600 hover:bg-blue-500 text-white flex items-center gap-2 shadow-sm transition">
            ...
            <div class="mt-12 flex justify-end">
                <div class="text-center text-xs space-y-1 w-64">
                    ...
                    <div class="h-20 flex items-center justify-center">
                        <span class="text-slate-300 italic text-[10px]">[ Tanda Tangan & Cap ]</span>
                    </div>
                    <p class="font-bold text-slate-900 underline">
                        {{ auth()->user()->nama ?? 'Administrator' }}
                    </p>
                    <p class="text-[10px] text-slate-500 font-mono">
                        ID: ADM-{{ str_pad(auth()->id() ?? 1, 4, '0', STR_PAD_LEFT) }}
                    </p>
                </div>
            </div>
        </div>
