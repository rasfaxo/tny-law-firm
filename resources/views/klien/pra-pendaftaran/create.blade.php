<x-app-layout title="Buat Pengajuan Perkara" :breadcrumbs="[['label' => 'Klien'], ['label' => 'Pengajuan', 'url' => route('klien.pra-pendaftaran.index')], ['label' => 'Buat']]">
    ...
    <div class="max-w-6xl mx-auto space-y-6" x-data="caseSubmission">
        <!-- Error Alert -->
        @if ($errors->any())
                        ...
                        <template x-for="(doc, index) in dokumenList" :key="doc.id">
                            <div class="space-y-5 p-5 border border-[#E2E8F0] bg-[#F8FAFC]/30 rounded-xl relative">
                                ...
                                <div class="flex justify-between items-center border-b border-[#E2E8F0] pb-3">
                                    ...
                                    <button type="button" aria-label="Hapus dokumen" @click="removeDokumen(index)" x-show="dokumenList.length > 1" class="text-rose-500 hover:bg-rose-50 p-1.5 rounded-lg transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500" title="Hapus Dokumen">
