    ...
    <div class="space-y-5">
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-xl p-5 shadow-sm space-y-3 no-print">
            ...
            <div class="flex flex-wrap gap-2.5">
                ...
                <a href="{{ route('admin.laporan.pengajuan-selesai') }}" class="inline-flex items-center gap-2 bg-navy-primary text-white border border-navy-primary text-sm font-semibold px-4 py-2 rounded-xl transition shadow-xs">
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-xl p-6 shadow-sm no-print">
            <form method="GET" action="{{ route('admin.laporan.pengajuan-selesai') }}" class="space-y-5">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <div>
                        <x-input-label for="tanggal_mulai" :value="__('Tanggal Awal Selesai')" />
                        <x-text-input type="date" id="tanggal_mulai" name="tanggal_mulai" :value="$filters['tanggal_mulai'] ?? ''" class="w-full text-navy-dark" />
                    </div>
                    <div>
                        <x-input-label for="tanggal_selesai" :value="__('Tanggal Akhir Selesai')" />
                ...
                <div class="flex flex-wrap items-center gap-3 pt-4 border-t border-[#E2E8F0]">
                    ...
                    <x-secondary-button href="{{ route('admin.laporan.pengajuan-selesai') }}" tag="a" class="gap-2">
                    ...
                    <a href="{{ route('admin.laporan.pengajuan-selesai.cetak', request()->query()) }}" target="_blank" rel="noopener noreferrer" class="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl h-11 px-5 text-sm font-semibold transition shadow-sm inline-flex items-center gap-2 ml-auto">
                        <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-xl shadow-sm overflow-hidden print-area">
            <div class="p-6 border-b border-[#F1F5F9] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h3 class="font-bold text-navy-dark text-lg">{{ __('Laporan Pengajuan Selesai') }}</h3>
                ...
                <table class="min-w-full divide-y divide-[#E2E8F0]">
                    <thead class="bg-[#F8FAFC]">
                        <tr>
                            ...
                            <th class="px-5 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Tanggal Selesai</th>
                    ...
                    <tbody class="bg-white divide-y divide-[#E2E8F0]">
                        @forelse ($laporan as $pengajuan)
                            ...
                            <tr class="hover:bg-[#F8FAFC] transition duration-150">
                                ...
                                <td class="px-5 py-4 whitespace-nowrap text-xs text-gray-500">
                                    {{ $tanggalSelesai?->format('d M Y H:i') ?? '-' }} WIB
                                </td>
