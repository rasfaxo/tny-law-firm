...
if ($target.Host -ne 'retest.tnypartners.com') {
    throw "Unexpected target '$($target.Host)'. This harness is locked to retest.tnypartners.com."
}
if ([string]::IsNullOrWhiteSpace($env:RETEST_PROVIDER_AUTHORIZATION)) {
    throw 'Set RETEST_PROVIDER_AUTHORIZATION to the written Rumahweb authorization reference.'
}
...
$runRoot = if ($outputLeaf -match '^\d{8}-\d{6}-rumahweb-v100$') { $OutputRoot } else { Join-Path $OutputRoot "$runId-rumahweb-v100" }
...
function Invoke-MultipartHttp {
    ...
    $client.DefaultRequestHeaders.Authorization = [Net.Http.Headers.AuthenticationHeaderValue]::new('Basic', $basicToken)
