            ...
            <x-card>
                <form method="POST" action="{{ route('admin.kategori-perkara.update', $kategoriPerkara) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-text-input tag="textarea" id="deskripsi" name="deskripsi" rows="4" class="mt-1 block w-full px-4 py-3">{{ old('deskripsi', $kategoriPerkara->deskripsi) }}</x-text-input>
                        <x-input-error class="mt-2" :messages="$errors->get('deskripsi')" />
                    </div>

                    <div class="flex items-center justify-end gap-3 pt-2">
                        <x-secondary-button href="{{ route('admin.kategori-perkara.show', $kategoriPerkara) }}" tag="a">
                            {{ __('Batal') }}
                        </x-secondary-button>
