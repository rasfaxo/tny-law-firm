    ...
    <div class="space-y-6">
        ...
        <div class="text-center space-y-1">
            ...
            <p class="text-xs text-slate-600">
                Rekapitulasi Riwayat Pemeriksaan Berkas Perkara oleh Tim Staf Legal
            </p>
        </div>
        ...
        <div class="bg-slate-50 border border-slate-300 rounded-lg p-3.5 text-xs text-slate-700 grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div>
                <span class="block text-[10px] font-semibold text-slate-500 uppercase">Staf Legal / Status</span>
                <span class="font-bold text-slate-900">
                    {{ $selectedStafLegal->nama ?? 'Semua Staf Legal' }}
                    @if ($filters['status_verifikasi'] ?? null)
            ...
            <table class="w-full border-collapse border border-slate-400 text-xs">
                <thead>
                    <tr class="bg-slate-100 text-slate-800 font-semibold">
                        ...
                        <th class="border border-slate-400 px-3 py-2 text-center w-28">Hasil Verifikasi</th>
                        <th class="border border-slate-400 px-3 py-2 text-center w-24">Tgl Verifikasi</th>
                    ...
                    @forelse ($laporan as $verifikasi)
                        <tr>
                            ...
                            <td class="border border-slate-400 px-3 py-2 font-mono font-bold text-blue-900">
                                PP-{{ str_pad($verifikasi->praPendaftaranPerkara?->id_pendaftaran ?? $verifikasi->id_pendaftaran, 3, '0', STR_PAD_LEFT) }}
                            </td>
                            <td class="border border-slate-400 px-3 py-2 font-medium text-slate-900">
                                {{ $verifikasi->praPendaftaranPerkara?->klien?->nama ?? '-' }}
                            </td>
                            <td class="border border-slate-400 px-3 py-2 text-slate-800">
                                {{ $verifikasi->praPendaftaranPerkara?->kategori?->nama_kategori ?? '-' }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="border border-slate-400 px-3 py-6 text-center text-slate-500 italic">
                                Tidak ada data riwayat verifikasi berkas yang sesuai dengan kriteria filter.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
